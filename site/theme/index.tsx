  ...
  useEffect(() => {
    function convertDates() {
      ...
      for (const el of targets) {
        ...
        // Idempotency: skip if already in target format (prevents observer loop)
        const hasHE = /\b1\d{4}\b/.test(text);
        if (holocene && hasHE) continue;
        if (!holocene && !hasHE) continue;

        if (holocene) {
          el.textContent = text.replace(/\b((?:19|20)\d{2})\b/g, (_m, y) => String(Number(y) + 10000));
        } else {
          el.textContent = text.replace(/\b1((?:19|20)\d{2})\b/g, (_m, y) => y);
        }
      }
    }
    ...
    // Re-run on SPA route changes (LastUpdated re-renders without holocene changing)
    const observer = new MutationObserver(convertDates);
